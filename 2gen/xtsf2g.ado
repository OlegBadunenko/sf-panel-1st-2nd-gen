...
// 		xtsf2g depvar [indepvars] [if] [in] [, options]
...
capture program drop xtsf2g
program define xtsf2g, eclass
version 14

  if !replay() {

  syntax varlist(numeric fv min=2) [if] [in]                  ///
		[, noCONStant COST MODEL(string) Distribution(string) ///
...
//   display "`distribution'"
  ...
  else if "`model'" == "K1990" | "`model'" == "" {
    local model 1
    local modelcoefs "beta[t]:gamma beta[t]:delta"
    local modelfn "beta[t] = ( 1 + exp(gamma*t + delta*t^2) )^-1"
  }
  ...
  else if "`model'" == "BC1992" {
    local model 3
    local modelcoefs "beta[t]:gamma"
    local modelfn "beta[t] = exp( -gamma*(t-T_i) )"
  }
  ...
  local cnames "`cnames' `modelcoefs'"

  mata: xtsf2g_vykonaty("`depvar'", "`indepvars'", "`touse'", "`constant'",  ///
    "`mypanelvar'",  "`mytimevar'", "`model'", "`cost'", "`celessthanone'",  ///
...
//   display `shat1'
  ...
  ereturn local predict "xtsf2g_p"
	ereturn local cmd   "xtsf2g"
...
//     display "replay here"
  ...
  if "`nolog'" == "" {
    ...
    display as input "{hline 29}"
		...
		display as input _newline " The second-generation estimator of"
...
capture mata mata drop xtsf2g_vykonaty()

mata:

void xtsf2g_vykonaty( string scalar depvar,                                  ///
  string scalar indepvars, string scalar touse,  string scalar constant, ///
  string scalar mypanelvar, string scalar mytimevar, ///
  string scalar model0, string scalar cost, string scalar celessthanone, ///
